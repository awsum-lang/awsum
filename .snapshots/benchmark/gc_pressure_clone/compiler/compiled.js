"use strict";

(() => {
  const __print = s => {
    process.stdout.write(String(s));
    return [0];
  };

  const __predInt32 = x => x === -2147483648 ? [3, [17]] : [4, x - 1 | 0];

  const __eqInt32 = (a, b) => a === b ? [1] : [2];

  const __addInt32 = (a, b) => {
    const r = a + b;
    if (r > 2147483647) {
      return [3, [882564211, [18]]];
    }
    if (r < -2147483648) {
      return [3, [3768445577, [17]]];
    }
    return [4, r | 0];
  };

  const v_zero = 0 | 0;

  const v_sumList = (v_lst, v_acc) => {
    while (true) {
      switch (v_lst[0]) {
        case 24: {
          return v_acc;
        }
        case 25: {
          {
            const __s = __addInt32(v_acc, v_lst[1]);
            switch (__s[0]) {
              case 3: {
                return v_acc;
              }
              case 4: {
                const v_next = __s[1];
                v_lst = v_lst[2];
                v_acc = v_next;
                continue;
              }
            }
          }
        }
      }
    }
  };

  const v_runIO = v_io => {
    while (true) {
      switch (v_io[0]) {
        case 5: {
          return v_io[1];
        }
        case 7: {
          const v_$inl0$eff = __print(v_io[1]);
          v_io = v_io[2];
          continue;
        }
      }
    }
  };

  const v_$apply$repeat = (v_$k, v_$x) => {
    while (true) {
      switch (v_$k[0]) {
        case 26: {
          return v_$x;
        }
        case 27: {
          const v_$pk__27 = v_$k[1];
          v_$x = (v_$k[0] = 25, v_$k[1] = v_$k[2], v_$k[2] = v_$x, v_$k);
          v_$k = v_$pk__27;
          continue;
        }
      }
    }
  };

  const v_$cps$repeat = (v_n, v_value, v_$k) => {
    while (true) {
      {
        const __s = __eqInt32(v_n, v_zero);
        switch (__s[0]) {
          case 1: {
            return v_$apply$repeat(v_$k, [24]);
          }
          case 2: {
            {
              const __s = __predInt32(v_n);
              switch (__s[0]) {
                case 3: {
                  return v_$apply$repeat(v_$k, [24]);
                }
                case 4: {
                  const v_m = __s[1];
                  v_n = v_m;
                  v_$k = [27, v_$k, v_value];
                  continue;
                }
              }
            }
          }
        }
      }
    }
  };

  const v_$apply$clone = (v_$k, v_$x) => {
    while (true) {
      switch (v_$k[0]) {
        case 28: {
          return v_$x;
        }
        case 29: {
          const v_$pk__29 = v_$k[1];
          v_$x = (v_$k[0] = 25, v_$k[1] = v_$k[2], v_$k[2] = v_$x, v_$k);
          v_$k = v_$pk__29;
          continue;
        }
      }
    }
  };

  const v_$cps$clone = (v_lst, v_$k) => {
    while (true) {
      switch (v_lst[0]) {
        case 24: {
          return v_$apply$clone(v_$k, v_lst);
        }
        case 25: {
          const v_x = v_lst[1];
          const v_xs = v_lst[2];
          v_$k = [29, v_$k, v_x];
          v_lst = v_xs;
          continue;
        }
      }
    }
  };

  const v_cloneN = (v_times, v_lst) => {
    while (true) {
      {
        const __s = __eqInt32(v_times, v_zero);
        switch (__s[0]) {
          case 1: {
            return v_lst;
          }
          case 2: {
            {
              const __s = __predInt32(v_times);
              switch (__s[0]) {
                case 3: {
                  return v_lst;
                }
                case 4: {
                  const v_m = __s[1];
                  v_times = v_m;
                  v_lst = v_$cps$clone(v_lst, [28]);
                  continue;
                }
              }
            }
          }
        }
      }
    }
  };

  const main = [
    7,
    String(
      v_sumList(
        v_cloneN(3000 | 0, v_$cps$repeat(100000 | 0, 1 | 0, [26])),
        v_zero
      )
    ),
    [5, [0]]
  ];

  if (typeof require !== "undefined" && require.main === module) {
    if (typeof main !== "undefined") {
      v_runIO(main);
    }
  }
})();
