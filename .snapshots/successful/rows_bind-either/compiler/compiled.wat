...
;;     at block + 12 and we want to write at block + 8).
  ...
  (func $v_run (param $v_x i32) (result i32)
    ...
    (local $__scrut i32)
    (local $__drop_tmp i32)
    (block (result i32) (local.set $__scrut (call $v_opA (block (result i32) (local.set $__inc_tmp (local.get $v_x)) (call $__inc_ref (local.get $__inc_tmp)) (local.get $__inc_tmp)))) (if (result i32) (i32.eq (i32.load (local.get $__scrut)) (i32.const 3)) (then (local.set $v__do_e_20_3 (i32.load offset=4 (local.get $__scrut))) (call $__inc_ref (local.get $v__do_e_20_3)) (block (result i32) (local.set $__drop_tmp (block (result i32) (i32.store (local.tee $__con_0 (call $__alloc_shaped (i32.const 8) (i32.const 1))) (i32.const 3)) (i32.store offset=4 (local.get $__con_0) (block (result i32) (i32.store (local.tee $__con_1 (call $__alloc_shaped (i32.const 8) (i32.const 1))) (i32.const 2252990199)) (i32.store offset=4 (local.get $__con_1) (local.get $v__do_e_20_3)) (call $__inc_ref (i32.load offset=4 (local.get $__con_1))) (local.get $__con_1))) (local.get $__con_0))) (call $__free_recursive (local.get $v__do_e_20_3)) (local.get $__drop_tmp))) (else (local.set $v_a (i32.load offset=4 (local.get $__scrut))) (call $__inc_ref (local.get $v_a)) (block (result i32) (local.set $__drop_tmp (call $v__lift_12 (call $v_opB (block (result i32) (local.set $__inc_tmp (local.get $v_a)) (call $__inc_ref (local.get $__inc_tmp)) (local.get $__inc_tmp))))) (call $__free_recursive (local.get $v_a)) (local.get $__drop_tmp))))))
