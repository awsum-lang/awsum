...
(() => {
  ...
  const v_$inl1$box = [24, "from box"];
  const v_res = (s => {
    switch (s[0]) {
      case 3: {
        const v_$do__e__0 = s[1];
        return [3, v_$do__e__0];
      }
      case 4: {
        const v_s0 = s[1];
        const v_$inl2$box = [24, [25, "nested"]];
        const v_$inl3$r = v_$inl2$box[1];
        return __concat(v_s0, v_$inl3$r[1]);
      }
    }
  })(__concat(v_$inl1$box[1], " "));
