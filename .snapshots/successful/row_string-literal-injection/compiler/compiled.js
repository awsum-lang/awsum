"use strict";

(() => {
  const __print = s => {
    process.stdout.write(String(s));
    return [0];
  };

  const v_sv = [1615808600, "S"];

  const v_runIO = v_io => {
    while (true) {
      switch (v_io[0]) {
        case 5: {
          return v_io[1];
        }
        case 7: {
          const v_$inl0$eff = __print(v_io[1]);
          v_io = v_io[2];
          continue;
        }
      }
    }
  };

  const v_iv = [2711245919, 9 | 0];

  const v_$apply$$df$andThenIO$8 = (v_$k, v_$x) => {
    while (true) {
      switch (v_$k[0]) {
        case 12: {
          return v_$x;
        }
        case 13: {
          const v_$pk__13 = v_$k[1];
          v_$x = (v_$k[0] = 7, v_$k[1] = v_$k[2], v_$k[2] = v_$x, v_$k);
          v_$k = v_$pk__13;
          continue;
        }
      }
    }
  };

  const v_$cps$$df$andThenIO$8 = (v_io, v_$k) => {
    while (true) {
      switch (v_io[0]) {
        case 5: {
          const v_$inl18$x = [1615808600, "T"];
          return v_$apply$$df$andThenIO$8(
            v_$k,
            [
              7,
              (s => {
                switch (s[0]) {
                  case 1615808600: {
                    return v_$inl18$x[1];
                  }
                  case 2711245919: {
                    return String(v_$inl18$x[1]);
                  }
                }
              })(v_$inl18$x),
              [5, [0]]
            ]
          );
        }
        case 7: {
          const v_s = v_io[1];
          const v_next = v_io[2];
          v_$k = [13, v_$k, v_s];
          v_io = v_next;
          continue;
        }
      }
    }
  };

  const v_$apply$$df$andThenIO$4 = (v_$k, v_$x) => {
    while (true) {
      switch (v_$k[0]) {
        case 10: {
          return v_$x;
        }
        case 11: {
          const v_$pk__11 = v_$k[1];
          v_$x = (v_$k[0] = 7, v_$k[1] = v_$k[2], v_$k[2] = v_$x, v_$k);
          v_$k = v_$pk__11;
          continue;
        }
      }
    }
  };

  const v_$cps$$df$andThenIO$4 = (v_io, v_$k) => {
    while (true) {
      switch (v_io[0]) {
        case 5: {
          const v_$inl21$x = [2711245919, 1 | 0];
          return v_$apply$$df$andThenIO$4(
            v_$k,
            [
              7,
              (s => {
                switch (s[0]) {
                  case 1615808600: {
                    return v_$inl21$x[1];
                  }
                  case 2711245919: {
                    return String(v_$inl21$x[1]);
                  }
                }
              })(v_$inl21$x),
              [5, [0]]
            ]
          );
        }
        case 7: {
          const v_s = v_io[1];
          const v_next = v_io[2];
          v_$k = [11, v_$k, v_s];
          v_io = v_next;
          continue;
        }
      }
    }
  };

  const v_$apply$$df$andThenIO$12 = (v_$k, v_$x) => {
    while (true) {
      switch (v_$k[0]) {
        case 14: {
          return v_$x;
        }
        case 15: {
          const v_$pk__15 = v_$k[1];
          v_$x = (v_$k[0] = 7, v_$k[1] = v_$k[2], v_$k[2] = v_$x, v_$k);
          v_$k = v_$pk__15;
          continue;
        }
      }
    }
  };

  const v_$cps$$df$andThenIO$12 = (v_io, v_$k) => {
    while (true) {
      switch (v_io[0]) {
        case 5: {
          const v_$inl24$x = v_iv;
          return v_$apply$$df$andThenIO$12(
            v_$k,
            [
              7,
              (s => {
                switch (s[0]) {
                  case 1615808600: {
                    return v_$inl24$x[1];
                  }
                  case 2711245919: {
                    return String(v_$inl24$x[1]);
                  }
                }
              })(v_$inl24$x),
              [5, [0]]
            ]
          );
        }
        case 7: {
          const v_s = v_io[1];
          const v_next = v_io[2];
          v_$k = [15, v_$k, v_s];
          v_io = v_next;
          continue;
        }
      }
    }
  };

  const v_$apply$$df$andThenIO$0 = (v_$k, v_$x) => {
    while (true) {
      switch (v_$k[0]) {
        case 8: {
          return v_$x;
        }
        case 9: {
          const v_$pk__9 = v_$k[1];
          v_$x = (v_$k[0] = 7, v_$k[1] = v_$k[2], v_$k[2] = v_$x, v_$k);
          v_$k = v_$pk__9;
          continue;
        }
      }
    }
  };

  const v_$cps$$df$andThenIO$0 = (v_io, v_$k) => {
    while (true) {
      switch (v_io[0]) {
        case 5: {
          const v_$inl27$lv = [1615808600, "L"];
          return v_$apply$$df$andThenIO$0(
            v_$k,
            [
              7,
              (s => {
                switch (s[0]) {
                  case 1615808600: {
                    return v_$inl27$lv[1];
                  }
                  case 2711245919: {
                    return String(v_$inl27$lv[1]);
                  }
                }
              })(v_$inl27$lv),
              [5, [0]]
            ]
          );
        }
        case 7: {
          const v_s = v_io[1];
          const v_next = v_io[2];
          v_$k = [9, v_$k, v_s];
          v_io = v_next;
          continue;
        }
      }
    }
  };

  const v_$inl32$x = v_sv;
  const main = v_$cps$$df$andThenIO$0(
    v_$cps$$df$andThenIO$4(
      v_$cps$$df$andThenIO$8(
        v_$cps$$df$andThenIO$12(
          [
            7,
            (s => {
              switch (s[0]) {
                case 1615808600: {
                  return v_$inl32$x[1];
                }
                case 2711245919: {
                  return String(v_$inl32$x[1]);
                }
              }
            })(v_$inl32$x),
            [5, [0]]
          ],
          [14]
        ),
        [12]
      ),
      [10]
    ),
    [8]
  );

  if (typeof require !== "undefined" && require.main === module) {
    if (typeof main !== "undefined") {
      v_runIO(main);
    }
  }
})();
