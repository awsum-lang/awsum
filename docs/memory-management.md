...
The same move semantics fires on the loop back-edge. At a `CContinue`, a `pending` binder — a parameter, a `let`, or a case-arm binder — that is forwarded as a bare borrowed `CVar` argument *exactly once* is moved into its destination parameter slot on LLVM and WASM: its existing reference is donated, so codegen elides both the forward `__inc_ref` and the paired `CDrop` free the uniform discipline would otherwise emit. The load-bearing extract-inc that protects an arm binder from its dying scrutinee's cascade stays — only the redundant inc/free pair is cut. Both preconditions are load-bearing: linearity, because one donated reference has exactly one destination (a binder forwarded to two slots keeps the ordinary inc-per-use plus single drop); and drop-set membership, because with no paired drop to cancel, skipping the inc would leak. This is the back-edge analogue of the value-tail move above. See `transferMoves` in [Awsum.Codegen.LLVM](../src/Awsum/Codegen/LLVM.hs) and `transferMovesW` in [Awsum.Codegen.WASM.Assemble](../src/Awsum/Codegen/WASM/Assemble.hs).

## Drop lowering per backend

- **LLVM**: `CDrop _ n body` lowers to `call void @__free_recursive(ptr %n)` after `body`'s value is produced — except a binder linearly moved across a `CContinue` back-edge (above), whose free is elided along with its forward inc. See [Awsum.Codegen.LLVM](../src/Awsum/Codegen/LLVM.hs).
