...
**Identifiers** are ASCII (`[A-Za-z0-9_']`, starting with a letter or `_`) and capped at **1024 characters**, rejected at parse time past that. A generous sanity bound — far beyond any hand-written name — that keeps a mangled identifier well inside every backend's symbol-length limit (the JVM's 65535-byte `CONSTANT_Utf8` again). Names the compiler synthesises internally are bounded by the same reasoning but never appear in source.
