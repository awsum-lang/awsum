...
--
-- 'builtInRegistry' is the single source of truth, and every entry
-- declares two things: the built-in's surface type and whether evaluating
-- a call to it is observable I/O ('Effectful') or pure ('Pure'). The type
-- drives the typechecker and lowering (via 'builtIns'); the flag drives
-- 'Awsum.Core.effectfulIn' (via 'effectfulBuiltIns'), which gates the
-- purity-sensitive drops in 'Awsum.Simplify'. Co-locating the flag with the
-- declaration is deliberate: adding a built-in forces the Pure/Effectful
-- choice here, so a new effect can't silently slip past the simplifier —
-- which, given a stray pure marking, would legally delete it.
--
...
-- | A built-in's effect status: whether evaluating a call to it performs
--   observable I/O. The privileged platform primitives 'runIO' invokes are
--   'Effectful'; every other built-in is a 'Pure' function of its arguments.
data Effectfulness = Pure | Effectful
  deriving stock (Show, Eq)

-- | The authoritative built-in registry, keyed by the name after
--   @BuiltIn.@ in source. Every entry declares the built-in's surface type
--   /and/ its effect status, so the two derived views below ('builtIns',
--   'effectfulBuiltIns') cannot drift from it — and adding a built-in forces
--   the 'Pure' \/ 'Effectful' choice at the declaration site.
builtInRegistry :: Map Name (Type', Effectfulness)
builtInRegistry =
  M.fromList
    [ ("showInt32", (TyArrow noSpan int32Ty stringTy, Pure)),
      ("showUInt8", (TyArrow noSpan uint8Ty stringTy, Pure)),
      ...
      ("byteToHexStringNoPrefix", (TyArrow noSpan uint8Ty stringTy, Pure)),
      -- predInt32 : Int32 -> Either UnderflowError Int32
      -- Returns `Left UnderflowError` on 'minInt32', `Right (x - 1)` elsewhere.
      ("predInt32", (TyArrow noSpan int32Ty (eitherTy underflowErrorTy int32Ty), Pure)),
      -- predUInt8 : UInt8 -> Either UnderflowError UInt8
      -- Returns `Left UnderflowError` on 0, `Right (x - 1)` elsewhere.
      ("predUInt8", (TyArrow noSpan uint8Ty (eitherTy underflowErrorTy uint8Ty), Pure)),
      -- succInt32 : Int32 -> Either OverflowError Int32
      -- Returns `Left OverflowError` on 'maxInt32', `Right (x + 1)` elsewhere.
      ("succInt32", (TyArrow noSpan int32Ty (eitherTy overflowErrorTy int32Ty), Pure)),
      -- succUInt8 : UInt8 -> Either OverflowError UInt8
      -- Returns `Left OverflowError` on 255, `Right (x + 1)` elsewhere.
      ("succUInt8", (TyArrow noSpan uint8Ty (eitherTy overflowErrorTy uint8Ty), Pure)),
      -- eqInt32 : Int32 -> Int32 -> Bool
      ("eqInt32", (TyArrow noSpan int32Ty (TyArrow noSpan int32Ty boolTy), Pure)),
      -- eqUInt8 : UInt8 -> UInt8 -> Bool
      ("eqUInt8", (TyArrow noSpan uint8Ty (TyArrow noSpan uint8Ty boolTy), Pure)),
      ...
      ("eqString", (TyArrow noSpan stringTy (TyArrow noSpan stringTy boolTy), Pure)),
      ...
      ("addInt32", (TyArrow noSpan int32Ty (TyArrow noSpan int32Ty (eitherTy arithRowTy int32Ty)), Pure)),
      ...
      -- 'addInt32'.
      ("subInt32", (TyArrow noSpan int32Ty (TyArrow noSpan int32Ty (eitherTy arithRowTy int32Ty)), Pure)),
      ...
      ("mulInt32", (TyArrow noSpan int32Ty (TyArrow noSpan int32Ty (eitherTy arithRowTy int32Ty)), Pure)),
      ...
      ("negInt32", (TyArrow noSpan int32Ty (eitherTy overflowErrorTy int32Ty), Pure)),
      ...
      ("addUInt8", (TyArrow noSpan uint8Ty (TyArrow noSpan uint8Ty (eitherTy overflowErrorTy uint8Ty)), Pure)),
      ...
      ("subUInt8", (TyArrow noSpan uint8Ty (TyArrow noSpan uint8Ty (eitherTy underflowErrorTy uint8Ty)), Pure)),
      ...
      ("mulUInt8", (TyArrow noSpan uint8Ty (TyArrow noSpan uint8Ty (eitherTy overflowErrorTy uint8Ty)), Pure)),
      ...
      ("concatString", (TyArrow noSpan stringTy (TyArrow noSpan stringTy (eitherTy stringTooLongTy stringTy)), Pure)),
      ...
      ("splitOnFirst", (TyArrow noSpan stringTy (TyArrow noSpan stringTy (maybeTy (tuple2Ty stringTy stringTy))), Pure)),
      ...
      ("parseInt32", (TyArrow noSpan stringTy (eitherTy parseErrorTy int32Ty), Pure)),
      -- parseUInt8 : String -> Either ParseError UInt8
      -- Same grammar, no sign accepted (UInt8 is unsigned), range 0..255.
      ("parseUInt8", (TyArrow noSpan stringTy (eitherTy parseErrorTy uint8Ty), Pure)),
      -- showUInt32 : UInt32 -> String
      ("showUInt32", (TyArrow noSpan uint32Ty stringTy, Pure)),
      -- predUInt32 : UInt32 -> Either UnderflowError UInt32
      -- Returns `Left UnderflowError` on 0, `Right (x - 1)` elsewhere.
      ("predUInt32", (TyArrow noSpan uint32Ty (eitherTy underflowErrorTy uint32Ty), Pure)),
      -- succUInt32 : UInt32 -> Either OverflowError UInt32
      -- Returns `Left OverflowError` on 4294967295, `Right (x + 1)` elsewhere.
      ("succUInt32", (TyArrow noSpan uint32Ty (eitherTy overflowErrorTy uint32Ty), Pure)),
      -- eqUInt32 : UInt32 -> UInt32 -> Bool
      ("eqUInt32", (TyArrow noSpan uint32Ty (TyArrow noSpan uint32Ty boolTy), Pure)),
      ...
      ("addUInt32", (TyArrow noSpan uint32Ty (TyArrow noSpan uint32Ty (eitherTy overflowErrorTy uint32Ty)), Pure)),
      -- subUInt32 : UInt32 -> UInt32 -> Either UnderflowError UInt32
      -- `Left UnderflowError` if `a < b`, `Right (a - b)` otherwise.
      -- Symmetric to 'subUInt8'.
      ("subUInt32", (TyArrow noSpan uint32Ty (TyArrow noSpan uint32Ty (eitherTy underflowErrorTy uint32Ty)), Pure)),
      -- mulUInt32 : UInt32 -> UInt32 -> Either OverflowError UInt32
      -- `Left OverflowError` if `a * b > 4294967295`, `Right (a * b)`
      -- otherwise. Symmetric to 'mulUInt8'.
      ("mulUInt32", (TyArrow noSpan uint32Ty (TyArrow noSpan uint32Ty (eitherTy overflowErrorTy uint32Ty)), Pure)),
      ...
      ("parseUInt32", (TyArrow noSpan stringTy (eitherTy parseErrorTy uint32Ty), Pure)),
      ...
      ("lengthCodePoints", (TyArrow noSpan stringTy uint32Ty, Pure)),
      ...
      ("lengthUtf16CodeUnits", (TyArrow noSpan stringTy uint32Ty, Pure)),
      ...
      ("lengthUtf8Bytes", (TyArrow noSpan stringTy uint32Ty, Pure)),
      ...
      ("internalStdoutPrint", (TyArrow noSpan stringTy unitTy, Effectful)),
      ...
      ("internalGetArgs", (eitherTy argvDecodeRowTy listStringTy, Effectful)),
      ...
      ("internalStdinReadAllString", (eitherTy stdinDecodeRowTy stringTy, Effectful)),
      ...
      ("internalStdinReadAllBytes", (listUInt8Ty, Effectful))
    ]
  where
...
--   after @BuiltIn.@ in source. The view the typechecker and lowering
--   consume — a projection of 'builtInRegistry'.
builtIns :: Map Name Type'
builtIns = fst <$> builtInRegistry

-- | Names whose evaluation is observable I/O, derived from the 'Effectful'
--   flag in 'builtInRegistry' so the set cannot drift from the
--   declarations. The single source for 'Awsum.Core.effectfulIn'.
effectfulBuiltIns :: Set Name
effectfulBuiltIns = M.keysSet (M.filter ((== Effectful) . snd) builtInRegistry)
