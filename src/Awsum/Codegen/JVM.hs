...
getArgsMethod ptags =
  T.intercalate "\n"
    ...
    <> makeNullaryCellLines (ptNil ptags)
    <> [ "  astore_2",
         "  aload_0",
         "  arraylength",
         "  istore_1",
         ...
         "  isub",
         "  istore_1",
         -- validated := __entryArgEither(argv[i])
         "  aload_0",
         "  iload_1",
         "  aaload",
         "  invokestatic AwsumMain/__entryArgEither(" <> objDesc <> ")" <> objDesc,
         "  astore_3",
         -- if validated[0] != Right tag, return validated (Left propagates)
         "  aload_3",
         "  iconst_0",
         "  aaload",
         "  checkcast java/lang/Integer",
         "  invokevirtual java/lang/Integer/intValue()I",
         pushIntInsn (ptRight ptags),
         "  if_icmpne L_args_left",
         -- list := Cons (validated[1]) list
         --   build Object[3] = [Cons tag, validated[1], list], leave on stack
         "  iconst_3",
         "  anewarray java/lang/Object",
         "  dup",
         "  iconst_0"
       ]
    <> boxedTagLines (ptCons ptags)
    <> [ "  aastore",
         "  dup",
         "  iconst_1",
         "  aload_3",
    ...
    <> makeUnaryCellFromLocalLines (ptRight ptags) "  aload_2"
    <> [ "  areturn",
         ".end method"
       ]
...
mainMethod =
  unlines
    [ ".method public static main([Ljava/lang/String;)V",
      ...
      -- Stash the entire 'args' reference in the '__argv' static field
      -- so '__getArgs' (called from 'runIO''s 'IOGetArgs' arm) can walk
      -- every element on demand. 'main' itself takes no arguments
      -- (its signature is 'IO Never Unit'); user code reads argv via
      -- 'IO.Args.getArgs' inside the IO chain. Argv is invariant for
      -- the JVM process lifetime, so one store at entry is enough.
      "  aload_0",
