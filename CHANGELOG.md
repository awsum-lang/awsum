...
- **CLR stdout on Windows with piped/redirected output** no longer collapses surrogate pairs to `??`. The CLR `Main` entry now sets `Console.OutputEncoding = System.Text.Encoding.UTF8` before any user code runs; the previous default fell back to the system ANSI code page when stdout wasn't a console, which encoded each UTF-16 code unit individually so a single supplementary code point became two `?` bytes. The fix is two `call` instructions at the top of `Main`, applied identically in both the textual codegen (`Codegen/CLR.hs`) and the binary PE assembler (`Codegen/CLR/Assemble.hs`).
