...
- **Hindley-Milner type inference.** The typechecker now drives expected types deeper into expressions. A bare integer in a `case`-arm body picks up its type from the enclosing function's signature, so
  ...
  typechecks without an extra `zero : Int32` binding. A constructor application inherits its argument types from the outer expected type — `Right 1 : Either ErrA Int32` typechecks without an annotation on the literal. The unifier is two-way and runs an occurs check, so infinite-type constructions like `α ~ α -> Int32` are rejected (the previous one-way matcher silently accepted them).

- **`do`-notation for `Either` chains.** A `do` block desugars to nested `bindEither` calls and runs end-to-end on all five backends:

  ```awsum
  do
    x <- parseInt32 raw1
    y <- parseInt32 raw2
    pureEither (addInt32 x y)
  ```

  There is no `pure` keyword: the trailing expression is whatever the user wrote, typically `pureEither` from the prelude. The desugarer rejects `let`, non-variable patterns, and binds whose right-hand side isn't an `Either`. New prelude entries: `bindEither`, `pureEither`, `mapRight`, `mapLeft`, `const`. Lambda syntax `\x -> body` is also part of the surface language now; a lambda that captures an outer-function parameter still needs a manual top-level helper, but `do`-block continuations lift cleanly.

- **Structural sums `T1 | T2`.** A new closed, anonymous union of types — usable in any signature position (`f : (Int32 | String) -> String`). Discriminated by type-ascription patterns `(x : T)` in `case` arms, exhaustively checked. Implicit injection at call sites: `f "hi"` works without an explicit wrapper. Mixed pattern arms — type-ascription and constructor — coexist in a single `case`, and a value flows into a wider row through nominal heads (e.g. `Left ErrA : Either ErrA r` is accepted where `Either (ErrA | ErrB) Int32` is required). Runtime dispatch is by FNV-1a hash of the canonical label name; LLVM / JVM / CLR / WASM / JS produce identical stdout. **Known limitations.** Row polymorphism is not yet wired — an open row tail cannot be a type variable. Greedy unification has corner cases on rows with type variables. Lifting a nominal `Either ErrB Int32` into a row-typed `Either (ErrA | ErrB) Int32` still requires a user-written wrapper.

- **Eta-reduced (point-free) top-level definitions.** A definition with no parameters on the LHS is accepted whenever its signature has arrow shape and the RHS already has the full function type — e.g. `say : String -> IO Unit; say = IO.Stdout.print`, or aliasing another top-level. Previously this only worked for the `BuiltIn.foo` form (which keeps its zero-cost erasure path); the lowering eta-expands every other RHS into a regular first-order function so the invariant "`CBuiltIn` only appears in callee position of `CCall`" still holds and every backend handles the alias as a normal call. Defining with *more* parameters than the signature allows is still an `ArityMismatch` error.

- **Property-based tests across all backends** — new `awsum-test` group `Property tests` plus `just test-property` target. Each property has an Awsum source under [test/sources/property/](test/sources/property/), a QuickCheck generator and an expected-output function in [test/Awsum/PropertySpec.hs](test/Awsum/PropertySpec.hs); the framework compiles the source once per property, then feeds N constructively-generated inputs through every backend and asserts that stdout is identical to the Haskell-computed expected output. **29 starter properties** covering integer arithmetic (commutativity, associativity, zero identity on both sides, agreement with Haskell on no-overflow inputs), succ/pred (round-trip + boundary failure-iff-extreme), equality (reflexive, symmetric for Int32 / UInt8), parser/show round-trip for both numeric types, the string monoid laws (left + right identity, associativity), splitOnFirst round-trip in both positive (sep present) and negative (sep absent) branches, and boolean laws (involution of not, commutativity of and / or, De Morgan). All 29 green on every backend; full run is ~145 s. Also extracts compile + run helpers into [test/Awsum/RunBackend.hs](test/Awsum/RunBackend.hs) (`Backend`, `CompiledArtifacts`, `compileFromFile`, `runOn`, `runOnAll`) shared with snapshot tests.
...
- **`RowCatchAllPattern` diagnostic now points at the `_` itself, not the surrounding `case` arm.** Editor squigglies underline the wildcard precisely.

- **`IOUnit` renamed to `IO Unit`.** The built-in atomic type constructor `IOUnit` is gone; effectful programs now write `main : String -> IO Unit` (the `Unit` half is the prelude-defined single-constructor type, the `IO` half is a new built-in unary type constructor). Affects every `main` signature and `IO.Stdout.print`'s declared type. The motivation is to stop hiding the effect type inside an opaque atom — `IO a` makes the parameter explicit, leaving room for future effectful return types without another atom per shape. All compiler-visible code paths (`Awsum.Typing` well-formedness + `requireMain`, `Awsum.Program.Cli`'s platform table, `MainWrongType` error message), the docs, and every `.aww` test file under `test/sources/` have been updated; snapshot golden files were regenerated.
...
- **CLR `InvalidProgramException` on deeply nested constructors.** Surfaced by the new `either-nested-right-300` / `either-nested-right-case-300` snapshot tests. Three independent bugs in [src/Awsum/Codegen/CLR/Assemble.hs](src/Awsum/Codegen/CLR/Assemble.hs), all triggered together at depths around `Right (Right (Right ...))` ≥ 5–10:
  ...
  - Reproducer (no longer reproduces): `awsum run -t clr test/sources/successful/either-nested-right-300/code/Main.aww`.
