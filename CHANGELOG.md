...
- **`awsum lsp --stdio` subcommand — Language Server Protocol.** New CLI entry that runs an LSP server backed by the same parser, typechecker, formatter, and quick-fix payload as every other `awsum` command. Implements document sync (`didOpen` / `didChange` debounced 500 ms / `didSave` / `didClose`), `publishDiagnostics`, `codeAction`, `formatting`, `documentSymbol`, and `workspace/symbol`; advertises its version through the `initialize` response's `serverInfo` field, so editors can detect lockstep mismatches without a separate `awsum --version` spawn. The transport flag is mandatory — `--stdio` is the only one today, future transports (e.g. `--socket PORT`) will land as additional mutually-exclusive flags, no defaulting. Designed to back editor extensions without direct compiler-API access (Zed) and to replace the temp-file-per-call CLI dance in `awsum-vscode` once it migrates to a thin LSP client.
