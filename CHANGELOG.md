...
- **A function whose JVM bytecode would exceed the 65535-byte per-method limit (`code_length`, JVM Spec §4.7.3) is now refused at compile time instead of emitted as an invalid `.class`.** Previously `awsum build -t jvm` wrote a class whose oversized method the JVM rejected only at load (`java.lang.ClassFormatError: Invalid method Code length`) — the program built but crashed when run, while LLVM/CLR/WASM/JS ran it fine, a runtime divergence. The binary assembler now measures each method's actual `code_length` (after every Core-to-Core pass and codegen) and, when one is over the cap, fails the build with a diagnostic naming the method and its size: `JVM target — function `v_main` compiles to 66069 bytes, over the JVM's hard limit of 65535 bytes per method. This program can't be built for the JVM target.` This is a per-target compile-time limit: the equivalence guarantee is about runtime behaviour, and refusing a program that exceeds one backend's hard ceiling is the honest response — capable backends are not capped to match (see [docs/targets.md](docs/targets.md#per-target-compile-time-limits)). Reaching the cap takes a pathologically large single function (e.g. a multi-thousand-element literal lowered into one straight-line body); ordinary code is nowhere near it.
