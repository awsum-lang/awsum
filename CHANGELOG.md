...
- **Snapshot test for string-literal escape sequences** — [test/sources/successful/strings_escapes](test/sources/successful/strings_escapes) prints a literal exercising `\n \t \r \" \\` and asserts identical bytes across all five backends (the 0x09 / 0x0A / 0x0D / 0x22 / 0x5C output bytes are pinned in the no-stdin golden). The sixth supported escape `\0` is intentionally excluded from this test: LLVM and WASM treat strings as null-terminated bytes and truncate at NUL, while JVM, CLR, and JS preserve it — a known cross-backend divergence that requires a length-prefixed string representation to fix and is tracked separately.
