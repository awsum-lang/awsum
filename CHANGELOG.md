...
- **Integer constant-folding semantics now lives in one place.** The op set, bounds and overflow/underflow outcomes of the foldable integer built-ins moved out of `Awsum.Simplify.constFold` into a new `Awsum.ConstEval` module that the `constFold-differential` property oracle reads too — one definition instead of two hand-synced copies, leaving the per-target runtime helpers as the only second implementation (validated against it by the differential). Generated code and program behaviour are unchanged; a new `constFold-coverage` test fails if a foldable built-in is added without a matching property-generator case.

### Fixed

- **A JVM-target function with more than 255 parameters is now refused at compile time instead of silently producing a corrupt `.class`.** A method descriptor encodes at most 255 parameter units (JVM Spec §4.3.3, §4.11) — for these all-`Object`, static methods, 255 parameters — and past that the descriptor is invalid, but the count was written unchecked, so `awsum build -t jvm` exited 0 and the JVM then rejected the artifact at load with `ClassFormatError: Too many arguments in method signature`, while the other four backends compiled and ran the same program. Reachable through code generation — defunctionalization prepends a closure's captures to a function's parameter list and the per-arity `$applyN` dispatchers carry the arity — so the check covers synthesised methods too. `awsum build -t jvm` now refuses such a program with a diagnostic naming the function and its parameter count, the sixth JVM class-file limit guarded this way (joining the per-method code-length cap and the four u2 fields). The other four backends impose no comparable limit and are unaffected.
- **The refusal diagnostic for an over-limit compiler-*synthesised* JVM method now labels it as synthetic and explains its origin.** The provenance hint (a fused `$scc$…` mutual-recursion group, or a `$cps$…` continuation-passing form) tested the raw `$scc$` / `$cps$` Core prefix against the already-mangled `v_$scc$…` method name, so it never matched and the hint never appeared; it now matches the mangled name. Only the wording of an existing per-target refusal changes.
...
- **Implicit row-widening through a non-regular (nested) recursive type now compiles instead of hanging the compiler.** A value of `Nest Bool` flowing into a position wanting `Nest (Bool | Unit)` (with `type Nest a = Deeper (Nest (Maybe a)) | Base a`) previously sent `check` / `build` / `awsum lsp` into an unbounded loop — the coercion synthesizer keyed each step on the instantiated type pair, which grows without bound for a nested type. The coercion is now synthesized as a parametric map derived once per type name (`$map$T`), terminating and producing identical output on all five backends; regular types keep the existing per-pair coercion unchanged. A non-regular type that carries its parameter in a contravariant (function-argument) position — where no such map exists — is rejected with a compile-time diagnostic rather than diverging.
- **Implicit row-widening through a non-regular type hidden behind a mutually-recursive partner no longer hangs the compiler.** `type A a = MkA (B (Maybe a)) | …` with `type B a = MkB (A a) | …` is non-regular — `B` re-applies `A` to a growing `Maybe a` — but the detector recognised only *direct* self-application, so the widening went to the per-pair coercion synthesiser, which loops on the unbounded `A Bool → B (Maybe Bool) → A (Maybe Bool) → …` chain (`check` / `build` / `awsum lsp` hung with no diagnostic). Non-regularity is now detected over the whole mutual-recursion cluster (the type's strongly-connected component in the type-dependency graph), so the coercion is derived as a terminating parametric map — completing the fix that previously covered only the single-type case. Regular mutually-recursive types are unaffected.
- **Row-widening through a covariant function field of a non-regular recursive type now compiles, and the contravariant case is rejected with an honest diagnostic.** A non-regular type with a function field `X -> a` — the parameter only in the result, e.g. `Hold (Int32 -> a)` — was rejected even though a map exists; it is now derived by post-composing the result coercion onto the stored function, mirroring the regular-type path. A field with the parameter in a contravariant or invariant (argument) position still has no finite map and is rejected — the regular-type case now reports a clear "function field … contravariant (argument) position" diagnostic instead of a raw internal `synthCoerce` message.
- **A user-defined row-widening function used as a value — passed to a higher-order function, stored in a constructor field, or returned — now widens correctly on all five backends.** A function whose type injects a free type variable into a row (`a -> (a | Unit)`, `IO e Unit -> IO (e | Err) Unit`, and the nominal-head and non-regular-recursive variants) was specialised per concrete row only when called directly; used as a value it reached the backend with its generic body, and the injection over the still-abstract variable was dropped as a no-op — a silent miscompile (LLVM trap, JVM/CLR/WASM the wrong alternative, JS `undefined`), differing across backends on some shapes. Row monomorphisation now fires in value positions too, specialising the function at the concrete row and cascading through enclosing definitions. Calling such functions directly was always correct, as were the prelude combinators (`bindEither`, `mapIOError`, …), which inject concrete constructors rather than a bare variable.
- **Over-long names can no longer silently corrupt the JVM artifact.** Synthesised top-level names compound across the flattening passes (`$scc$` joins an SCC's members, `$cps$` / `$apply$` and `$df$` wrap further), so deep mutual recursion through defunctionalised higher-order functions could push a single name past the JVM's 65535-byte `CONSTANT_Utf8` limit — a corrupt `.class` with `build` exiting 0, while LLVM / JS / WASM (which tolerate long names) compiled it fine. A final lowering pass now rewrites any synthesised name over 128 bytes to a bounded `<prefix>$x$<hash>` form and repoints every reference; output is unchanged and only the few fused-recursion names that crossed the threshold are affected. Source identifiers are likewise capped (1024 characters, rejected at parse time), so a hand-written name can't overflow a backend either.
- **A program exceeding the JVM's class-file size limits is now refused at compile time instead of silently producing a corrupt `.class`.** Three 16-bit class-file fields were written through an unchecked `fromIntegral` that wrapped on overflow: `constant_pool_count` (reached by a program of roughly 10,000+ functions — an ordinary codebase size), a method's `max_stack` / `max_locals`, and a single `CONSTANT_Utf8_info` length (a function with thousands of parameters, whose method descriptor overflows the field). Each wrapped silently with `build` exiting 0, and the JVM then rejected the artifact at load with `ClassFormatError`, while the other four backends compiled and ran the same program. `awsum build -t jvm` now refuses such a program with a diagnostic naming the limit and the figure that crossed it — the discipline already applied to the per-method 65535-byte code-length cap. The other four backends impose no comparable limit and are unaffected.
- **A program whose CLR metadata outgrows 16-bit indices no longer silently produces a corrupt `.dll`.** Every `#~`-table index into the metadata heaps and tables was emitted as 2 bytes (the `HeapSizes` byte hard-coded to 0, name/signature indices truncated on the way in), so a program whose `#Strings` heap — every method and type name — passed 64 KB had its name indices wrap: `build` exited 0, then `dotnet` threw `TypeLoadException` on a garbled name, while LLVM / JVM / WASM / JS ran the same program. The index widths are now derived per ECMA-335 §II.24.2.6 from the final heap sizes and row counts — heap indices (`#Strings` / `#GUID` / `#Blob`), simple table indices (`MethodList` → MethodDef, `ParamList` → Param), and coded indices (`MemberRefParent`, which widens at ~8192 methods regardless of heap size) — emitting 4-byte fields where required. Because ECMA-335 supports the wider encoding, CLR now compiles arbitrarily large programs like the other backends rather than refusing them as the JVM must. Output for programs under the boundary is byte-identical.
- **A recursive type with no non-recursive base (e.g. `type Loop = MkLoop Loop`) is now correctly treated as uninhabited, so a constructor carrying it as a field is excused from `case` coverage.** Inhabitance was computed as a greatest fixpoint — re-entering a recursive type mid-check assumed it inhabited — so a baseless recursive type was wrongly deemed inhabited and a `case` was forced to handle a constructor that can never be built (rejected with `Non-exhaustive … missing Wrap`). It is now a least fixpoint: a recursive self-reference yields no value on its own, so inhabitance requires a non-recursive base to bottom out — matching a strict language's actual inhabitance and the existing "uninhabited fields excuse coverage" rule (as for `Box Never`). Genuinely inhabited recursive types (`List`, …) are unaffected — they bottom out at their base constructor.
- **Lowering-phase diagnostics now point at the offending source location instead of the start of the file.** A lowering error — chiefly an unsupported row-widening coercion (a contravariant or invariant function field, or a non-regular recursive type) — was carried as a bare message with no span, so `check --json` reported it at `1:1` and an editor click landed at the top of the file. The `TELowering` error now carries an optional span, threaded from the offending type; the internal-invariant assertions a user cannot provoke keep the no-location form, and the two messages that hand-formatted a `line:col` into their text now use the span field.
