...
- **`awsum lsp --stdio` subcommand — Language Server Protocol.** Runs an LSP server reusing the existing parser, typechecker, formatter, and quick-fix payload. Implements document sync (debounced 500 ms), `publishDiagnostics`, `codeAction`, `formatting`, `documentSymbol`, `workspace/symbol`. Opt-in lockstep version check via `initializationOptions: { expectedAwsumVersion, preferButtonsOverLinks }` — on mismatch the server surfaces `window/showMessageRequest` (action button + `window/showDocument` for clients with reliable external-URL handling, inline URL otherwise; non-`A.B.C` versions opt out). Transport flag mandatory, no defaulting; `--stdio` is the only one today, future transports (e.g. `--socket PORT`) land as additional mutually-exclusive flags.
...
- **`Awsum.LowerClosures` Core-to-Core pass** — Reynolds defunctionalization for residual function values: closures that survive `Awsum.Defunctionalize` (stored in constructor fields, passed through case-arm-binders, captured into partial applications inside non-statically-resolvable HOFs) are encoded as tagged `CCon` values and routed through per-arity `$applyN` dispatchers. After the pass no first-class function value remains in Core; every backend sees only top-level fn calls or `$applyN` calls. Pipeline position: between `Awsum.Defunctionalize` (with its tree-shake) and `saturateProgram`, before Saturate's "no partial application with local captures" invariant would otherwise break for programs that store closures in constructor fields. Three snapshot tests pin the new pass: [closures_function-in-constructor-field](test/sources/successful/closures_function-in-constructor-field) (basic encoding + dispatch with two distinct closures), [closures_function-in-constructor-field-non-tail-stress](test/sources/successful/closures_function-in-constructor-field-non-tail-stress) (depth 1_000_000 non-tail self-recursion with `$apply1` in the post-call continuation, verifies `Awsum.Cps` composes through synthetic dispatchers), and [closures_function-in-constructor-field-mutual-stress](test/sources/successful/closures_function-in-constructor-field-mutual-stress) (depth 1_000_000 mutual recursion with the cycle passing through `$apply1`, verifies `Awsum.Scc` detects the SCC across the synthetic vertex).
- **`IO.Args.getArgs : IO (StringTooLong | UnpairedUtf16Surrogate) String`** — CLI platform built-in that reads the command-line argument from inside the IO chain. Decoding failures live in the IO error row and compose with `handleErrorIO` like any other IO failure. Each backend's runtime helper (`__entryArgEither`) runs two checks in a single pass: length-cap (combined UTF-16 length compared against `maxStringLengthUtf16CodeUnits = 2^27`; on UTF-8 backends the byte scan short-circuits at `cap+1`, bounding adversarial-input cost at `O(min(len, 3×cap))`) and strict UTF-16 (rejects unpaired surrogates — UTF-16 backends walk code units, UTF-8 backends reject the `ED A0..BF 80..BF` triplet pattern that WTF-8 / CESU-8 / Java modified UTF-8 emit). Cap-check has priority. Backed by a new `IOGetArgs` constructor in `IO` (first IO constructor with a function-typed field) and a privileged `BuiltIn.internalGetArgs` primitive that each backend re-reads on every call (no memoisation).

### Changed

- **Multi-line ADT declarations in `awsum format`.** A `type X = …` declaration with three or more constructors is rendered as `type X\n  = A\n  | B\n  | C`, with `=` and each subsequent `|` on its own line at indent 2; one or two constructors stay on the header line. The parser accepts both shapes (compact and multi-line continuations land at any column > 1) so hand-written single-line `type Foo = A | B | C` still parses and the formatter normalises it.
- **`main` signature is `IO Never Unit` (no parameter).** Programs read the entry-point argument via `IO.Args.getArgs` inside the IO chain.
...
- **Row-tag dispatch on case-binders whose source type can't be synthesised (`Awsum.ElaborateLower.synthLabelType` was incomplete).** Whenever a case-binder's type ended up as an un-substituted tyvar at lowering time, the row-injection wrapper around the rebuilt value fell back to `coercible`'s TyVar src rule and `find` picked the *first* label of the target row regardless of the value's actual runtime type — a row of `(UnderflowError | StringTooLong)` always dispatched as `UnderflowError`, a row of `(Int32 | String)` as `Int32`. Four broken synthesis paths fixed: (1) `EInfix OpConcat` had a stale `String` shortcut from before `(++)` was changed to `Either StringTooLong String` — now reuses `lookupBuiltIn "concatString"`; (2) `EInfix OpPipe` synthesises as the equivalent `EApp`; (3) non-nullary `ECon` returns its full arrow type instead of `Nothing`, with `EApp`'s clause unifying argument types against the parameter slots so a saturated `Just "hello"` resolves to `Maybe String` (and bails out to `Nothing` if the result still has free tyvars, so an unresolvable polymorphic shape doesn't trip downstream `synthCoerce`); (4) `ECase` synthesises from the first arm's body in an env extended with the arm's pattern binders, so a `case`-as-scrutinee chain lets the outer arm's binder receive the right runtime type. Four new snapshot tests pin the fixes per variant.
- **Constructor tags are globally unique across the program**, so `treeShakeFromMain` no longer over-approximates which `case` arms are reachable. Tags from synthetic sum types — SCC-merge `$args`, `LowerClosures` `$applyN` shapes, CPS `K_i` continuations — are allocated from the same monotonic supply seeded by `nextFreshConTag`, not from per-type 0-based counters that collided across types. Without this, an `Int` tag value appearing in any reachable `CCon` (e.g. tag 3 used by an SCC-merge dispatcher) kept the `IOGetArgs` arm of `runIO`/`bindIO`/`andThenIO` (also tag 3 in `IO`) live in programs that never imported `IO.Args`, leaving a residual `$apply1` call to a dispatcher whose body was an empty `CCase`. The empty `CCase` lowered to a malformed `phi ptr` with no incoming values that LLVM 15 (apt/brew/MSVC clang in CI) rejected with `expected '[' in phi value list`; older Apple clang and llvm-mingw 19 happened to accept it, masking the bug locally.
