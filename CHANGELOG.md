...
- **A row-combinator applied with an inline-lambda continuation in a type-checked position is now specialised correctly.** `bindEither oa (\_n -> pureEither 7)` against an explicit signature left the call head's error row abstract, so the injected error reached the `case` untagged.
